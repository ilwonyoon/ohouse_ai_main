/**
 * API: Process consultation message
 * POST /api/consultation/process
 *
 * Processes user message, extracts metadata using LLM, and generates response
 */

import { NextRequest, NextResponse } from "next/server";
import { extractMetadataFromMessage } from "@/api/metadataExtractor";
import { extractMetadataWithLLM, generateConsultantResponse } from "@/api/openai";
import { consultationEngine } from "@/api/consultationEngine";
import { ConsultationMessage, ExtractedMetadata } from "@/types/consultation";

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const {
      userMessage,
      consultationId,
      previousMetadata,
      currentPhase,
      messages = [],
    } = body;

    // Validation
    if (!userMessage || !consultationId) {
      return NextResponse.json(
        { error: "Missing required fields: userMessage, consultationId" },
        { status: 400 }
      );
    }

    // 1. Try to extract metadata using OpenAI first (sophisticated)
    // Fall back to pattern matching if OpenAI fails
    let extractedMetadata: ExtractedMetadata;
    let llmMetadata: Record<string, any> = {};

    try {
      // Convert messages to chat format for OpenAI
      const chatHistory = messages
        .filter((m: ConsultationMessage) => !m.extractedMetadata) // Avoid duplicating extraction data
        .map((m: ConsultationMessage) => ({
          role: m.role === "user" ? "user" : "assistant",
          content: m.content,
        }));

      // Extract metadata using LLM
      llmMetadata = await extractMetadataWithLLM(userMessage, chatHistory);
      console.log("LLM Extracted Metadata:", llmMetadata);
    } catch (llmError) {
      console.warn("LLM extraction failed, falling back to pattern matching:", llmError);
      // Fall back to pattern matching
      llmMetadata = {};
    }

    // 2. Also run pattern-based extraction for comparison
    const patternMetadata = await extractMetadataFromMessage(
      userMessage,
      previousMetadata
    );

    // 3. Merge LLM results with pattern matching (LLM takes priority)
    extractedMetadata = {
      ...patternMetadata,
      ...llmMetadata,
      confidence: Math.max(
        patternMetadata.confidence,
        Object.keys(llmMetadata).length > 0 ? 0.9 : 0
      ),
      rawKeywords: patternMetadata.rawKeywords,
    };

    // 4. Process user response and detect conversion signals
    const processedResponse = await consultationEngine.processUserResponse(
      userMessage,
      extractedMetadata
    );

    // 5. Determine if we should transition to next phase
    const shouldTransition = consultationEngine.shouldMoveToNextPhase(
      extractedMetadata,
      currentPhase || "intent_detection"
    );

    // 6. Determine next phase
    let nextPhase = currentPhase || "intent_detection";
    if (shouldTransition || processedResponse.conversionSignal) {
      nextPhase = consultationEngine.determineNextPhase(
        extractedMetadata.projectScope?.type || "exploratory"
      );
    }

    // 7. Generate response using OpenAI
    let assistantMessage = "";
    try {
      const chatHistory = messages.map((m: ConsultationMessage) => ({
        role: m.role === "user" ? "user" : "assistant",
        content: m.content,
      }));

      assistantMessage = await generateConsultantResponse(
        userMessage,
        chatHistory
      );
    } catch (openaiError) {
      console.warn("OpenAI generation failed, using fallback:", openaiError);
      // Fall back to template-based response
      const fallbackResponse = await consultationEngine.generateNextQuestion(
        messages,
        extractedMetadata,
        nextPhase
      );
      assistantMessage = fallbackResponse.conversationalMessage;
    }

    return NextResponse.json({
      success: true,
      data: {
        extractedMetadata,
        assistantMessage,
        nextPhase,
        shouldTransition,
        conversionSignal: processedResponse.conversionSignal,
        llmMetadata,
      },
    });
  } catch (error) {
    console.error("Error processing consultation message:", error);
    return NextResponse.json(
      {
        error: "Failed to process message",
        details: error instanceof Error ? error.message : "Unknown error",
      },
      { status: 500 }
    );
  }
}
