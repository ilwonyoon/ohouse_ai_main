...
// ===== MOODBOARD TYPES (Agent 1.5) =====

/**
 * Color palette extracted from image analysis
 * Used for design harmony and consistency
 */
export interface ColorPalette {
  primary: {
    hex: string;
    name: string;
    usage: string; // "walls", "furniture", "accents", etc.
  };
  secondary: Array<{
    hex: string;
    name: string;
  ...
  accents: Array<{
    hex: string;
    name: string;
    mood?: string; // "energetic", "calming", etc.
  }>;
...
/**
 * Design element or style component in moodboard
 */
export interface DesignElement {
  type: "furniture" | "lighting" | "textiles" | "decor" | "color_accent" | "material";
  name: string;
  description: string;
  style?: StyleCategory;
...
/**
 * Single image reference in moodboard
 * Generated from image API or curated from library
 */
export interface MoodboardImage {
  id: string;
  url: string;
  description: string; // What this shows and why it was chosen
  style: StyleCategory;
  dominantColors: string[]; // hex codes
  designElements: DesignElement[];
  visualMood: string; // "cozy", "modern", "luxurious", etc.
  confidence: number; // 0-1 confidence this matches user preferences
  source: "generated" | "curated" | "uploaded";
  generatedPrompt?: string; // Prompt used if generated by DALL-E
}

/**
 * Complete moodboard structure
 * Represents visual direction for the room design
 */
export interface Moodboard {
  ...
  // Core moodboard data
  title: string; // e.g., "Modern Minimalist Living Room"
  description: string; // Overall concept explanation
  concept: string; // Design narrative/story

  // Style information
  primaryStyle: StyleCategory;
  secondaryStyles: StyleCategory[];
  styleNarrative: string; // Explanation of style choices

  // Visual components
  colorPalette: ColorPalette;
  designElements: DesignElement[];
  moodboardImages: MoodboardImage[]; // 5-8 curated/generated images

  // Metadata
  mood: string; // "cozy", "energetic", "sophisticated", etc.
  atmosphere: string; // Detailed atmosphere description
  basedOnUserPreferences: Partial<StyleQuizResults>; // Reference to what user preferences informed this
  basedOnRoomAnalysis?: {
    roomType: string;
  ...
  // Generation metadata
  generationMethod: "ai_generated" | "curated" | "hybrid";
  aiModel?: "dall-e-3" | "stable-diffusion" | "custom";
  ...
  estimatedBudgetRange?: {
    ...
    breakdown: {
      ...
      textiles?: number;
      decor?: number;
      paint_and_walls?: number;
    };
  };
}

/**
 * Input parameters for moodboard generation
 */
...
/**
 * Moodboard generation response
 */
export interface MoodboardGenerationResponse {
  status: "success" | "error" | "in_progress";
  moodboard?: Moodboard;
  errorMessage?: string;
  generationTime?: number; // milliseconds
  imageGenerationTime?: number; // Time spent on DALL-E API calls
  progressPercentage?: number; // For async generation tracking
}
